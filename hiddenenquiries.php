...
<?php require_once('home/common/employeeheader.fly');//adding employee header  ?>
	...
	<?php
		...
		if($_SESSION['current_loggedin_user_role'] == 'admin')
			$query="SELECT * FROM `enquiries` WHERE `status` = 'Hidden' ORDER BY `id` DESC ";
		else
			$query="SELECT * FROM `enquiries` WHERE `status` = 'Hidden' AND `assignedto` = '$employee' ORDER BY `id` DESC ";
