...
<?php
//fetch blogs
$blogtype=mysqli_real_escape_string($dbconn,trim(strip_tags(stripslashes($_GET['blogtype']))));
$blogsearch=mysqli_real_escape_string($dbconn,trim(strip_tags(stripslashes($_GET['blogsearch']))));


	$query = "SELECT * FROM `blog` WHERE `blogtype` = '$blogtype' ";
	if($blogsearch!=""){
		$query.="AND (`author` like '%$blogsearch%' OR `about` like '%$blogsearch%') ";
	}
    $query.="order by `id` ;";
